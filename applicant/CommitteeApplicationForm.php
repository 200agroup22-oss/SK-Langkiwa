<?php
// Generic applicant-facing application form for any committee that doesn't have its own dedicated
// form page — i.e. every committee besides Education/Health/Sports. Committee is selected via
// ?committee=<id>, mirroring admin/CommitteeApplicants.php and admin/CommitteeForms.php.
...
$committee = null;
if ($committeeId > 0) {
    $stmt = $conn->prepare("SELECT * FROM committees WHERE committee_id = ? AND archived_at IS NULL");
...
if (!$committee) {
    header("Location: " . APP_BASE . "/applicant/applicant.php");
    exit();
}
...
// form below can display that specific program's name instead of just the committee's.
$programId = null;
$programName = null;
$programClosed = null;
$programRow = null;
...
$displayName = $programName ?: $committee['name'];
