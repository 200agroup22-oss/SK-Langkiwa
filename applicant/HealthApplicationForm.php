...
$displayName = $programName ?: 'Health';
...
$stmt = $conn->prepare("SELECT application_id, status FROM applications WHERE user_id = ? AND committee_id = ? AND program_id <=> ? AND status != 'declined' AND archived_at IS NULL LIMIT 1");
$stmt->bind_param('iii', $me['user_id'], $committeeId, $programId);
$stmt->execute();
...
if (!$existing && !$programClosed && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("INSERT INTO applications (user_id, committee_id, program_id, status, academic_year, semester) VALUES (?, ?, ?, 'pending', ?, ?)");
        $stmt->bind_param('iiiss', $me['user_id'], $committeeId, $programId, $settings['current_academic_year'], $settings['current_semester']);
        $stmt->execute();
        $applicationId = $stmt->insert_id;
        $stmt->close();

        saveDynamicSubmission($applicationId, $committeeId, $_POST, $_FILES, 'assistance', $programId);
    ...
    <div class="container-fluid" style="margin-top: 55px;">
        <div class="row">
            ...
            <div class="col-12 col-md-3 col-lg-2 sidebar order-2 order-md-1">
                <?php $activeLink = 'Health';
                include(__DIR__ . '/../includes/applicantformsidebar.php'); ?>
            </div>
