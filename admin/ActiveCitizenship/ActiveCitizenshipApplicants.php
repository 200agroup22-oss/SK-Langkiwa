...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_applicant'])) {
        ...
        if (empty($errors)) {
            ...
            logAudit('Added Applicant', 'Active Citizenship applicant #' . $applicationId);
    ...
    if (isset($_POST['edit_application'])) {
        ...
        if (empty($errors)) {
            ...
            logAudit('Updated Applicant', 'Active Citizenship applicant #' . $applicationId);
    ...
    if (isset($_POST['approve_application'])) {
        ...
        if (isProgramFull($committeeId, $track)) {
            ...
            $stmt->execute();
            $stmt->close();
            logAudit('Approved Application', 'Active Citizenship applicant #' . $applicationId);
    ...
    if (isset($_POST['decline_application'])) {
        ...
        $stmt->execute();
        $stmt->close();
        logAudit('Declined Application', 'Active Citizenship applicant #' . $applicationId);
    ...
    if (isset($_POST['archive_application'])) {
        ...
        $stmt->execute();
        $stmt->close();
        logAudit('Archived Applicant', 'Active Citizenship applicant #' . $applicationId);
    ...
    if (isset($_POST['restore_application'])) {
        ...
        $stmt->execute();
        $stmt->close();
        logAudit('Restored Applicant', 'Active Citizenship applicant #' . $applicationId);
        setFlash('success', 'Applicant restored.');
    }

    header("Location: ActiveCitizenshipApplicants.php" . ($programId !== null ? '?program_id=' . $programId : ''));
    exit();
}
...
$activeLink = 'ActiveCitizenshipApplicants';
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title><?php echo e($activeProgram ? $activeProgram['name'] : 'Active Citizenship'); ?> Applicants</title>
...
<body>
    ...
    <div class="main-content">
        <h4 class="fw-bold mb-1"><?php echo e($activeProgram ? $activeProgram['name'] : 'Active Citizenship'); ?> Applicants</h4>
        <p class="text-muted mb-2" style="font-size: 13px;">
            <?php if ($activeProgram): ?>
                Viewing applications for the <?php echo e($activeProgram['name']); ?> program only. <a href="ActiveCitizenshipApplicants.php">View all Active Citizenship programs</a>.
            <?php else: ?>
                View applications for Active Citizenship financial assistance.
            <?php endif; ?>
        </p>
            ...
            <div class="modal-content border-0 shadow">
                <form method="post" enctype="multipart/form-data">
                    <div class="modal-header">
                        <h6 class="modal-title fw-bold"><i class="bi bi-person-plus-fill me-2"></i> Active Citizenship Assistance Application Form</h6>
                ...
                <div class="modal-content border-0 shadow">
                    <form method="post">
                        ...
                        <div class="modal-body p-4 text-center">
                            ...
                            <p class="text-muted" style="font-size:12px; margin-bottom:0;">This applicant will be approved for Active Citizenship assistance.</p>
                        </div>
                        <div class="modal-footer border-0 justify-content-center gap-2">
                            <button type="button" class="btn btn-sm btn-secondary px-4" data-bs-dismiss="modal">Cancel</button>
