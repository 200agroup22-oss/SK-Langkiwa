...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_beneficiary'])) {
        ...
        if ($valid && $items !== '' && $quantity > 0) {
            ...
            $stmt->execute();
            $stmt->close();
            logAudit('Added In-Kind Beneficiary', 'Active Citizenship application #' . $applicationId);
...
<head>
    ...
    <title><?php echo e($activeProgram ? $activeProgram['name'] : 'Active Citizenship'); ?> In-Kind Assistance Beneficiaries</title>
...
<body>
    ...
    <div class="main-content">
        <h4 class="fw-bold mb-1"><?php echo e($activeProgram ? $activeProgram['name'] : 'Active Citizenship'); ?> In-Kind Assistance Beneficiaries</h4>
        <p class="text-muted mb-2" style="font-size: 13px;">
            <?php if ($activeProgram): ?>
                Viewing beneficiaries for the <?php echo e($activeProgram['name']); ?> program only. <a href="ActiveCitizenshipInKindAssistance.php">View all Active Citizenship programs</a>.
            <?php else: ?>
                Monitor beneficiaries approved for Active Citizenship in-kind assistance.
            <?php endif; ?>
        </p>
                ...
                <form method="post">
                    ...
                    <div class="modal-body p-4">
                        <?php if (empty($eligibleApplications)): ?>
                            <div class="alert alert-warning py-2 mb-0">There are no approved Active Citizenship applicants awaiting in-kind assistance setup right now.</div>
                        <?php else: ?>
