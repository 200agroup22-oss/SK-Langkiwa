...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_beneficiary'])) {
        ...
        if ($valid && $amount > 0 && applicationAssistanceTypeMatches($applicationId, $type)) {
            $stmt = $conn->prepare("INSERT INTO assistance_beneficiaries (application_id, type, amount, status) VALUES (?, ?, ?, 'pending')");
            $stmt->bind_param('isd', $applicationId, $type, $amount);
            $stmt->execute();
            $stmt->close();
            logAudit('Added Cash Beneficiary', $committee['name'] . ' application #' . $applicationId);
            setFlash('success', 'Beneficiary added.');
        } else {
            setFlash('error', 'Could not add beneficiary. Choose a valid applicant and amount.');
        }
    }
    ...
    if (isset($_POST['edit_beneficiary'])) {
        ...
        $amount = (float)($_POST['amount'] ?? 0);
        $status = in_array($_POST['status'] ?? '', ['pending', 'released'], true) ? $_POST['status'] : 'pending';
        $dateReleased = $status === 'released' ? ($_POST['date_released'] ?: date('Y-m-d')) : null;

        $stmt = $conn->prepare("UPDATE assistance_beneficiaries SET amount = ?, status = ?, date_released = ? WHERE beneficiary_id = ? AND type = 'cash'");
        $stmt->bind_param('dssi', $amount, $status, $dateReleased, $beneficiaryId);
        $stmt->execute();
        $stmt->close();
        logAudit('Updated Cash Beneficiary', 'Beneficiary #' . $beneficiaryId);
        setFlash('success', 'Beneficiary updated.');
    }

    if (isset($_POST['release_payment'])) {
        $beneficiaryId = (int)($_POST['beneficiary_id'] ?? 0);
        $dateReleased = $_POST['date_released'] ?: date('Y-m-d');

        $stmt = $conn->prepare("UPDATE assistance_beneficiaries SET status = 'released', date_released = ? WHERE beneficiary_id = ? AND type = 'cash'");
        $stmt->bind_param('si', $dateReleased, $beneficiaryId);
        $stmt->execute();
        $stmt->close();
        logAudit('Released Cash Payment', 'Beneficiary #' . $beneficiaryId);
        setFlash('success', 'Payment marked as released.');
    }

    if (isset($_POST['archive_beneficiary'])) {
        $beneficiaryId = (int)($_POST['beneficiary_id'] ?? 0);
        $stmt = $conn->prepare("UPDATE assistance_beneficiaries SET archived_at = NOW() WHERE beneficiary_id = ? AND type = 'cash'");
        $stmt->bind_param('i', $beneficiaryId);
        $stmt->execute();
        $stmt->close();
        logAudit('Archived Cash Beneficiary', 'Beneficiary #' . $beneficiaryId);
        setFlash('success', 'Beneficiary record archived.');
    }

    if (isset($_POST['restore_beneficiary'])) {
        $beneficiaryId = (int)($_POST['beneficiary_id'] ?? 0);
        $stmt = $conn->prepare("UPDATE assistance_beneficiaries SET archived_at = NULL WHERE beneficiary_id = ? AND type = 'cash'");
        $stmt->bind_param('i', $beneficiaryId);
        $stmt->execute();
        $stmt->close();
        logAudit('Restored Cash Beneficiary', 'Beneficiary #' . $beneficiaryId);
        setFlash('success', 'Beneficiary record restored.');
    }

    header("Location: CommitteeCashAssistance.php?committee=" . $committeeId . ($programId !== null ? '&program_id=' . $programId : ''));
    exit();
}
...
$sql = "SELECT a.application_id, a.user_id, a.program_id, ab.beneficiary_id, ab.amount, ab.status AS b_status, ab.date_released
        FROM applications a
...
$archivedStmt = $conn->prepare("SELECT ab.beneficiary_id, ab.application_id, ab.amount, ab.archived_at, a.program_id
    FROM assistance_beneficiaries ab
    JOIN applications a ON a.application_id = ab.application_id
    WHERE a.committee_id = ? AND a.program_track = ? AND ab.type = 'cash' AND ab.archived_at IS NOT NULL
...
switch ($sortBy) {
    ...
    case 'name_desc':
        ...
        break;
    case 'amount_asc':
        usort($displayRows, fn($a, $b) => (float)$a['amount'] <=> (float)$b['amount']);
        break;
    case 'amount_desc':
        usort($displayRows, fn($a, $b) => (float)$b['amount'] <=> (float)$a['amount']);
        break;
    case 'id_desc':
...
$activeLink = 'CommitteeCashAssistance_' . $committeeId;
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title><?php echo e($activeProgram ? $activeProgram['name'] : $committee['name']); ?> Cash Assistance Beneficiaries</title>
...
<body>
    ...
    <div class="main-content">
        <h4 class="fw-bold mb-1"><?php echo e($activeProgram ? $activeProgram['name'] : $committee['name']); ?> Cash Assistance Beneficiaries</h4>
        <p class="text-muted mb-2" style="font-size: 13px;">
            <?php if ($activeProgram): ?>
                Viewing beneficiaries for the <?php echo e($activeProgram['name']); ?> program only. <a href="CommitteeCashAssistance.php?committee=<?php echo $committeeId; ?>">View all <?php echo e($committee['name']); ?> programs</a>.
            <?php else: ?>
                Monitor beneficiaries approved for <?php echo e($committee['name']); ?> cash assistance.
            <?php endif; ?>
        </p>
        ...
        <form method="get">
            <input type="hidden" name="committee" value="<?php echo $committeeId; ?>">
            <!-- Table Header -->
            ...
            <div class="filter-bar d-flex gap-2 align-items-center mb-3 flex-wrap">
                ...
                <select class="filter-select" name="sort" onchange="this.form.submit()">
                    ...
                    <option value="amount_asc" <?php echo $sortBy === 'amount_asc' ? 'selected' : ''; ?>>Amount: Low &rarr; High</option>
                    <option value="amount_desc" <?php echo $sortBy === 'amount_desc' ? 'selected' : ''; ?>>Amount: High &rarr; Low</option>
                </select>
            </div>
        </form>
                ...
                <table class="table mb-0">
                    ...
                    <tbody>
                        <?php if (empty($pagedRows)): ?>
                            <tr>
                                <td colspan="<?php echo !empty($committeePrograms) ? 7 : 6; ?>" class="text-center text-muted py-4">No <?php echo e($activeProgram ? $activeProgram['name'] : $committee['name']); ?> cash assistance beneficiaries found.</td>
                            </tr>
                        <?php endif; ?>
                        <?php foreach ($pagedRows as $row): ?>
                            <tr>
                                ...
                                <td><?php echo $row['beneficiary_id'] ? '₱' . number_format((float)$row['amount'], 2) : '—'; ?></td>
                ...
                <form method="post">
                    <div class="modal-header">
                        <h6 class="modal-title fw-bold" id="addBeneficiaryModalLabel">
                            <i class="bi bi-person-plus-fill me-2"></i> Add Cash Assistance Beneficiary
                        </h6>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body p-4">
                        <?php if (empty($eligibleApplications)): ?>
                            <div class="alert alert-warning py-2 mb-0">There are no approved <?php echo e($committee['name']); ?> applicants awaiting cash assistance setup right now.</div>
                        <?php else: ?>
                            ...
                            <div class="section-divider"><i class="bi bi-cash-coin me-1"></i> Assistance Information</div>
                            <div class="row g-3 mb-3">
                                <div class="col-md-6">
                                    <label class="info-label">Amount <span class="text-danger">*</span></label>
    ...
    ?>
                    ...
                    <div class="modal-body p-4">
                        ...
                        <div class="row g-3 mb-2">
                            <div class="col-md-4">
                                ...
                                <div class="info-value">&#8369;<?php echo number_format((float)$row['amount'], 2); ?></div>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="info-value"><?php echo $row['date_released'] ? e($row['date_released']) : '—'; ?></div>
                            </div>
                        </div>
                        ...
                        <?php if ($row['b_status'] === 'pending'): ?>
                            <div class="decision-bar">
                                <div class="decision-title"><i class="bi bi-cash-stack me-1"></i> Payment Release</div>
                                <div class="decision-sub">Confirm this beneficiary has received their cash assistance.</div>
                                <div class="d-flex gap-2 flex-wrap">
                                    <button class="btn-approve" data-bs-dismiss="modal" data-bs-toggle="modal" data-bs-target="#releaseModal<?php echo $row['beneficiary_id']; ?>"><i class="bi bi-cash-coin"></i> Mark as Released</button>
                                </div>
                            </div>
                        <?php endif; ?>
                    </div>
                ...
                <div class="modal-content border-0 shadow">
                    <form method="post">
                        ...
                        <div class="modal-body p-4">
                            ...
                            <div class="row g-3 mb-2">
                                <div class="col-md-4">
                                    <label class="form-label" style="font-size:11px;font-weight:700;color:#888;text-transform:uppercase;">Amount</label>
                                    <div class="input-group input-group-sm">
                                        <span class="input-group-text">&#8369;</span>
                                        <input type="number" step="0.01" min="0.01" class="form-control" name="amount" value="<?php echo e($row['amount']); ?>" required>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    ...
                                    <select class="form-select form-select-sm" name="status">
                                        ...
                                        <option value="released" <?php echo $row['b_status'] === 'released' ? 'selected' : ''; ?>>Released</option>
                                    </select>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label" style="font-size:11px;font-weight:700;color:#888;text-transform:uppercase;">Date Released</label>
                                    <input type="date" class="form-control form-control-sm" name="date_released" value="<?php echo e($row['date_released']); ?>">
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer border-0">
                ...
                <div class="modal-content border-0 shadow">
                    <form method="post">
                        ...
                        <div class="modal-body p-4 text-center">
                            ...
                            <p class="text-muted" style="font-size:12px; margin-bottom:0;">This hides the cash assistance record for this applicant. The application itself is not affected, and you can restore it later from Archives.</p>
                        </div>
        ...
        <!-- RELEASE MODAL -->
        <div class="modal fade" id="releaseModal<?php echo $row['beneficiary_id']; ?>" tabindex="-1">
            <div class="modal-dialog modal-dialog-centered modal-sm">
                <div class="modal-content border-0 shadow">
                    <form method="post">
                        <input type="hidden" name="beneficiary_id" value="<?php echo $row['beneficiary_id']; ?>">
                        <div class="modal-header" style="background: linear-gradient(90deg, #45b84d, #aadaad);">
                            <h6 class="modal-title fw-bold text-white"><i class="bi bi-cash-coin me-2"></i>Release Payment</h6>
                        ...
                        <div class="modal-body p-4 text-center">
                            <div style="width:64px;height:64px;border-radius:50%;background:#e8f5e9;display:flex;align-items:center;justify-content:center;margin:0 auto 14px;">
                                <i class="bi bi-cash-stack" style="font-size:28px;color:#2e7d32;"></i>
                            </div>
                            <p class="fw-bold mb-1" style="font-size:14px;">Mark this beneficiary as released?</p>
                            <p class="text-muted" style="font-size:12px; margin-bottom:14px;">This confirms the cash assistance amount has been given to the beneficiary.</p>
                            <div class="text-start">
                                <label style="font-size:12px;font-weight:600;color:#555;">Date Released</label>
                                <input type="date" class="form-control form-control-sm mt-1" name="date_released" value="<?php echo date('Y-m-d'); ?>" required>
                            </div>
                        </div>
                        <div class="modal-footer border-0 justify-content-center gap-2">
                            <button type="button" class="btn btn-sm btn-secondary px-4" data-bs-dismiss="modal">Cancel</button>
                            <button type="submit" name="release_payment" class="btn btn-sm btn-success px-4"><i class="bi bi-check-lg me-1"></i>Confirm Release</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <div class="modal fade" id="archivesBeneficiaryModal" tabindex="-1">
        <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content border-0 shadow">
                <div class="modal-header">
                    <h6 class="modal-title fw-bold"><i class="bi bi-archive-fill me-2"></i> Archived Beneficiaries (<?php echo e($committee['name']); ?> Cash Assistance)</h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-4">
                    <div class="table-card">
                                ...
                                <tbody>
                                    ...
                                    <?php foreach ($archivedBeneficiaries as $ab): ?>
                                        <tr>
                                            ...
                                            <td>₱<?php echo number_format((float)$ab['amount'], 2); ?></td>
