...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete_beneficiary'])) {
        $beneficiaryId = (int)($_POST['beneficiary_id'] ?? 0);
        $stmt = $conn->prepare("DELETE FROM assistance_beneficiaries WHERE beneficiary_id = ? AND type = 'cash'");
        $stmt->bind_param('i', $beneficiaryId);
        $stmt->execute();
        $stmt->close();
        logAudit('Deleted Cash Beneficiary', 'Beneficiary #' . $beneficiaryId);
...
<body>
    ...
    ?>
        ...
        <!-- DELETE MODAL (repurposed Archive - assistance_beneficiaries has no archived_at column) -->
                ...
                <div class="modal-content border-0 shadow">
                    <form method="post">
                        ...
                        <div class="modal-body p-4 text-center">
                            ...
                            <p class="text-muted" style="font-size:12px; margin-bottom:0;">This removes the cash assistance record for this applicant. The application itself is not affected, and a new beneficiary record can be added again later.</p>
                        </div>
    ...
    <div class="modal fade" id="archivesBeneficiaryModal" tabindex="-1">
        <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content border-0 shadow">
                <div class="modal-header">
                    <h6 class="modal-title fw-bold"><i class="bi bi-list-ul me-2"></i> All Approved Applicants (<?php echo e($committee['name']); ?> Assistance)</h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-4">
                    <div class="table-card">
                        <div class="table-responsive-wrap">
                            <table class="table mb-0" style="font-size:13px;">
                                ...
                                <tbody>
                                    <?php if (empty($rows)): ?>
                                        <tr>
                                            <td colspan="5" class="text-center text-muted">No approved <?php echo e($committee['name']); ?> applicants yet.</td>
                                        </tr>
                                    <?php endif; ?>
