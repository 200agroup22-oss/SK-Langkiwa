...
// Every custom program added under Health's Assistance track has its own independent form — so
...
$baseTabLabel = getProgramTab($committeeId, 'assistance', 'Health Assistance Program')['label'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['save_field'])) {
        ...
        if ($label === '') {
            ...
            $stmt->execute();
            $stmt->close();
            logAudit('Added Form Field', $label . ($programId ? ' (' . $formHeading . ' only)' : ' (shared)'));
    ...
    header("Location: HealthForms.php" . ($viewingTab ? '?ptab=' . $viewingTab['tab_id'] : ''));
    exit();
}
...
<head>
    ...
    <title>Application Form - Health</title>
...
<body>
    ...
    <div class="main-content">
        ...
        <p class="text-muted mb-2" style="font-size: 13px;">
            ...
            <?php if ($viewingTab): ?>
                This form is independent from <strong><?php echo e($baseTabLabel); ?></strong> and every other Health program — it started as a copy of the base form, and changes here only affect <strong><?php echo e($formHeading); ?></strong>.
            <?php endif; ?>
        </p>
