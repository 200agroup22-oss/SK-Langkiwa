...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_beneficiary'])) {
        ...
        if ($valid && $amount > 0 && applicationAssistanceTypeMatches($applicationId, $type)) {
            ...
            $stmt->execute();
            $stmt->close();
            logAudit('Added Cash Beneficiary', 'Health application #' . $applicationId);
    ...
    header("Location: HealthCashAssistance.php" . ($programId !== null ? '?program_id=' . $programId : ''));
    exit();
}
...
$activeLink = 'HealthCashAssistance';
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title><?php echo e($activeProgram ? $activeProgram['name'] : 'Health'); ?> Cash Assistance Beneficiaries</title>
...
<body>
    ...
    <div class="main-content">
        <h4 class="fw-bold mb-1"><?php echo e($activeProgram ? $activeProgram['name'] : 'Health'); ?> Cash Assistance Beneficiaries</h4>
        <p class="text-muted mb-2" style="font-size: 13px;">
            <?php if ($activeProgram): ?>
                Viewing beneficiaries for the <?php echo e($activeProgram['name']); ?> program only. <a href="HealthCashAssistance.php">View all Health programs</a>.
            <?php else: ?>
                Monitor beneficiaries approved for Health cash assistance.
            <?php endif; ?>
        </p>
                ...
                <table class="table mb-0">
                    ...
                    <tbody>
                        <?php if (empty($pagedRows)): ?>
                            <tr>
                                <td colspan="<?php echo !empty($committeePrograms) ? 7 : 6; ?>" class="text-center text-muted py-4">No <?php echo e($activeProgram ? $activeProgram['name'] : 'health'); ?> cash assistance beneficiaries found.</td>
                            </tr>
                        <?php endif; ?>
                ...
                <form method="post">
                    ...
                    <div class="modal-body p-4">
                        <?php if (empty($eligibleApplications)): ?>
                            <div class="alert alert-warning py-2 mb-0">There are no approved Health applicants awaiting cash assistance setup right now.</div>
                        <?php else: ?>
    ...
    <div class="modal fade" id="archivesBeneficiaryModal" tabindex="-1">
        <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content border-0 shadow">
                <div class="modal-header">
                    <h6 class="modal-title fw-bold"><i class="bi bi-archive-fill me-2"></i> Archived Beneficiaries (Health Cash Assistance)</h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-4">
                    <div class="table-card">
