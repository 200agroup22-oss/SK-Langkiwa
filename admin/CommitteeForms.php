<?php
// Generic Form-builder page for any committee added beyond the 4 built-in ones.
...
// Every program tab under a generic committee has its own independent form — so reflect which
// program tab the admin actually clicked (via ?ptab=) in the heading instead of always showing
// just the committee name.
$viewingTab = isset($_GET['ptab']) ? getProgramTabById((int)$_GET['ptab']) : null;
$formHeading = $viewingTab['label'] ?? $committee['name'];
// null on the base view (manages the base form); a program's own id when viewing one of its tabs
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    header("Location: CommitteeForms.php?committee=" . $committeeId . ($viewingTab ? '&ptab=' . $viewingTab['tab_id'] : ''));
    exit();
}
...
$activeLink = 'CommitteeForms_' . $committeeId;
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>Application Form - <?php echo e($committee['name']); ?></title>
...
<body>
    ...
    <div class="main-content">
        ...
        <p class="text-muted mb-2" style="font-size: 13px;">
            ...
            <?php if ($viewingTab): ?>
                This form is independent from the base <strong><?php echo e($committee['name']); ?></strong> form and every other program under it — it started as a copy of the base form, and changes here only affect <strong><?php echo e($formHeading); ?></strong>.
            <?php endif; ?>
        </p>
