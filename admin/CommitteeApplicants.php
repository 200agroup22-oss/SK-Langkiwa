<?php
// Generic Applicants page for any committee added via Content Management beyond the 4 built-in
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_applicant'])) {
        ...
        if (empty($errors)) {
            ...
            logAudit('Added Applicant', $committee['name'] . ' applicant #' . $applicationId);
    ...
    if (isset($_POST['edit_application'])) {
        ...
        if (empty($errors)) {
            ...
            logAudit('Updated Applicant', $committee['name'] . ' applicant #' . $applicationId);
    ...
    if (isset($_POST['archive_application'])) {
        ...
        $stmt->execute();
        $stmt->close();
        logAudit('Archived Applicant', $committee['name'] . ' applicant #' . $applicationId);
    ...
    if (isset($_POST['restore_application'])) {
        ...
        $stmt->execute();
        $stmt->close();
        logAudit('Restored Applicant', $committee['name'] . ' applicant #' . $applicationId);
        setFlash('success', 'Applicant restored.');
    }

    header("Location: CommitteeApplicants.php?committee=" . $committeeId . ($programId !== null ? '&program_id=' . $programId : ''));
    exit();
}
...
<head>
    ...
    <title><?php echo e($activeProgram ? $activeProgram['name'] : $committee['name']); ?> Applicants</title>
...
<body>
    ...
    <div class="main-content">
        <h4 class="fw-bold mb-1"><?php echo e($activeProgram ? $activeProgram['name'] : $committee['name']); ?> Applicants</h4>
        <p class="text-muted mb-2" style="font-size: 13px;">
            <?php if ($activeProgram): ?>
                Viewing applications for the <?php echo e($activeProgram['name']); ?> program only. <a href="CommitteeApplicants.php?committee=<?php echo $committeeId; ?>">View all <?php echo e($committee['name']); ?> programs</a>.
            <?php else: ?>
                View applications for <?php echo e($committee['name']); ?> financial assistance.
            <?php endif; ?>
        </p>
            ...
            <div class="modal-content border-0 shadow">
                <form method="post" enctype="multipart/form-data">
                    <div class="modal-header">
                        <h6 class="modal-title fw-bold"><i class="bi bi-person-plus-fill me-2"></i> <?php echo e($committee['name']); ?> Assistance Application Form</h6>
                ...
                <div class="modal-content border-0 shadow">
                    <form method="post">
                        ...
                        <div class="modal-body p-4 text-center">
                            ...
                            <p class="text-muted" style="font-size:12px; margin-bottom:0;">This applicant will be approved for <?php echo e($committee['name']); ?> assistance.</p>
                        </div>
                        <div class="modal-footer border-0 justify-content-center gap-2">
                            <button type="button" class="btn btn-sm btn-secondary px-4" data-bs-dismiss="modal">Cancel</button>
