...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_applicant'])) {
        ...
        if (empty($errors)) {
            ...
            logAudit('Added Applicant', 'Sports applicant #' . $applicationId);
    ...
    if (isset($_POST['edit_application'])) {
        ...
        if (empty($errors)) {
            ...
            logAudit('Updated Applicant', 'Sports applicant #' . $applicationId);
    ...
    if (isset($_POST['archive_application'])) {
        ...
        $stmt->execute();
        $stmt->close();
        logAudit('Archived Applicant', 'Sports applicant #' . $applicationId);
    ...
    if (isset($_POST['restore_application'])) {
        ...
        $stmt->execute();
        $stmt->close();
        logAudit('Restored Applicant', 'Sports applicant #' . $applicationId);
        setFlash('success', 'Applicant restored.');
    }

    header("Location: SportsApplicants.php" . ($programId !== null ? '?program_id=' . $programId : ''));
    exit();
}
...
<head>
    ...
    <title>Committee on <?php echo e($activeProgram ? $activeProgram['name'] : 'Sports'); ?> - Applicants</title>
...
<body>
    ...
    <div class="main-content">
        <h4 class="fw-bold mb-1"><?php echo e($activeProgram ? $activeProgram['name'] : 'Sports Committee Financial Assistance'); ?> Applicants</h4>
        <p class="text-muted mb-2" style="font-size: 13px;">
            <?php if ($activeProgram): ?>
                Viewing applications for the <?php echo e($activeProgram['name']); ?> program only. <a href="SportsApplicants.php">View all Sports Committee programs</a>.
            <?php else: ?>
                View applications for Sports Committee financial assistance.
            <?php endif; ?>
        </p>
            ...
            <div class="modal-content border-0 shadow">
                <form method="post" enctype="multipart/form-data">
                    <div class="modal-header">
                        <h6 class="modal-title fw-bold"><i class="bi bi-person-plus-fill me-2"></i> Sports Committee Assistance Application Form</h6>
                ...
                <div class="modal-content border-0 shadow">
                    <form method="post">
                        ...
                        <div class="modal-body p-4 text-center">
                            ...
                            <p class="text-muted" style="font-size:12px; margin-bottom:0;">This applicant will be approved for Sports Committee assistance.</p>
                        </div>
                        <div class="modal-footer border-0 justify-content-center gap-2">
                            <button type="button" class="btn btn-sm btn-secondary px-4" data-bs-dismiss="modal">Cancel</button>
