...
function slugifyFieldKey($label, $committeeId, $track, $conn) {
    ...
    $i = 2;
    while (true) {
        $stmt = $conn->prepare("SELECT field_id FROM form_fields WHERE committee_id = ? AND program_track = ? AND field_key = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['save_field'])) {
        ...
        if ($label === '') {
            ...
            $fieldKey = slugifyFieldKey($label, $committeeId, $track, $conn);
            ...
            $stmt = $conn->prepare("INSERT INTO form_fields (committee_id, program_track, label, field_key, input_type, icon, width, is_required, options, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param('issssssisi', $committeeId, $track, $label, $fieldKey, $inputType, $icon, $width, $required, $options, $sortOrder);
            $stmt->execute();
            $stmt->close();
    ...
    if (isset($_POST['move_field'])) {
        ...
        if ($index !== null && isset($fields[$swapWith])) {
            $a = $fields[$index];
            $b = $fields[$swapWith];
    ...
    header("Location: EducationForms.php");
    exit();
}
...
<head>
    ...
    <title>Application Form - Education</title>
...
<body>
    ...
    <div class="main-content">
        <h4 class="fw-bold mb-1">Application Form — Education (iSKolar)</h4>
        <p class="text-muted mb-2" style="font-size: 13px;">Manage the fields applicants fill out on the public Education scholarship application form. Changes take effect immediately.</p>
                ...
                <form method="post">
                    ...
                    <div class="modal-body p-4">
                        ...
                        <div class="mb-3" id="optionsFieldWrap" style="display:none;">
                            ...
                            <input type="text" class="form-control form-control-sm" name="options" id="fieldOptionsInput" placeholder="e.g. 1st Year, 2nd Year, 3rd Year, 4th Year">
                        </div>
                        <div class="mb-3">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body p-4">
                    <div class="preview-form-card p-2">
                        <h5 class="text-center fw-bold mb-4" style="letter-spacing:0.5px;">SCHOLAR APPLICATION FORM</h5>
